...
my_pantry_id = os.getenv("PANTRY_ID")
my_basket = os.getenv("PANTRY_BASKET")

url = f"https://getpantry.cloud/apiv1/pantry/{my_pantry_id}/basket/{my_basket}"
...
class MyDisplay:
    ...
    def switch(self):
        ...
        self.disp.set_backlight(1 if self.backlight else 0)

    def check_time(self):
        hour = datetime.now().hour
        if (hour >= self.t0 and hour < self.t1) and not self.backlight:
            self.switch()
        elif (hour < self.t0 or hour >= self.t1) and self.backlight:
            self.switch()
...
disp = MyDisplay(t0=9, t1=19)

WIDTH = disp.disp.width
HEIGHT = disp.disp.height
...
while True:
    ...
    # Noise
    low, mid, high, amp = list(map(lambda x: x * 128, noise.get_noise_profile()))
    amp /= 2
